...
public class SucursalService {
	...
	public List<SucursalResponse> listarTodas() {
		// Esta linea es necesario explicar: .()stream es un metodo para las
		// colecciones, abre un flujo de datos, convierte la lista en un flujo
		// procesable
		// lo que permite ahorrarse el for para recorrer cada elemento y trabajar de
		// forma declarativa. Sin el .stream() se tendria que hacer un for tradicional.
		// .map()transforma cada elemento
		// .collect() sirve para recolectar todos los elementos transformados y los
		// guarda en una lista. Escrita por Cristian, para mejor entendimiento.
		return this.sucursalRepository.obtenerTodas().stream()
				.map(sucursal -> this.convertirSucursalAResponse(sucursal)).collect(Collectors.toList());
	}

	public HashMap<Integer, String> obtenerSucursalesDisponibles() {
		return this.sucursalRepository.obtenerSucursalesDisponibles();
	}
